...
function () {
   ...
   */
  function AjaxLoginWithCake(param) {
    _classCallCheck(this, AjaxLoginWithCake);
  ...
  _createClass(AjaxLoginWithCake, [{
    key: "_setParamIfEmpty",
    value: function _setParamIfEmpty(param) {
      if (param == null) param = {}; // ボタンタイプ  0:プレーンスタイル , 1:Bootstrapスタイル
    ...
    key: "_margeParam",
    value: function _margeParam(param) {
    ...
    key: "loginCheckEx",
    value: function loginCheckEx(param) {
    ...
    key: "loginCheck",
    value: function loginCheck(param) {
      var _this = this;
      ...
      $.ajax({
        ...
        _this._showBtns(res); // クライアントから指定されたコールバック関数を実行する
    ...
    key: "_getLogoutBtnHtml",
    value: function _getLogoutBtnHtml() {
    ...
    key: "_getLoginBtnHtml",
    value: function _getLoginBtnHtml() {
...
/*#__PURE__*/
function () {
  function CalendarViewK() {
    _classCallCheck(this, CalendarViewK);
  }

  _createClass(CalendarViewK, [{
    key: "create",
     ...
     */
    value: function create(data, date_field, callback) {
      // データ中の日付データを整形する
      data = this._shapingDateInData(data, date_field); // 基準月初日を取得する
      ...
      var base_month_end = this._getMonthEndDate(base_month_start); // カレンダーデータのフォーマットを作成する。


      var calData = this._createCalendarDataFormat(base_month_start, base_month_end); // データを日付で集計する


      var dataA = this._aggByDateField(data, date_field); // カレンダーデータに日付集計データをマッピングする


      calData = this._mappingToCalData(calData, dataA); // カレンダーデータの週ごとに構造変換する

      calData = this._weekStructConv(calData); // コールバック関数が空であるなら見本コールバック関数をセットする。

      if (callback == null) callback = this._callback; // カレンダービューHTMLを組み立てる
    ...
    key: "_callback",
    value: function _callback(cellData, self) {
      var html = "";
    ...
    key: "_getNameField",
    value: function _getNameField(ent) {
    ...
    key: "_xss_sanitize",
    value: function (_xss_sanitize2) {
      function _xss_sanitize(_x) {
        return _xss_sanitize2.apply(this, arguments);
      }

      _xss_sanitize.toString = function () {
        return _xss_sanitize2.toString();
      };

      return _xss_sanitize;
    }(function (data) {
    ...
    key: "_buildCalendarViewHtml",
    value: function _buildCalendarViewHtml(calData, callback, base_month_start) {
      // 年月を取得
      var dt1 = new Date(base_month_start);
      var ym = dt1.getFullYear() + '-' + (dt1.getMonth() + 1);
      var html = "<table class='calendar_view_k'><caption>" + ym + "</caption><thead><tr>"; // ▼ 列部分の日 ～ 土を作成
      ...
      for (var w_i in calData) {
        ...
        for (var key_date in wData) {
          var ent = wData[key_date]; // 日, 曜日番号, 曜日コードを取得する

          var cellDate = new Date(key_date);
          var day = cellDate.getDate();
          var week_no = cellDate.getDay();
          var week_code = weeks[week_no]['code']; // クラス属性・曜日

          var day_class = 'cvk_' + week_code; // クラス属性・当月外

          if (ent['cur_month_flg'] == 0) day_class = 'cvk_out_this_month'; // セル内容をコールバック関数にて作成

          var cell_contents = callback(ent, this);
          html += "<td><div class='cvk_cell'>" + "<div class='cvk_cell_div1 " + day_class + "'>" + day + "</div>" + "<div class='cvk_cell_div2'> " + week_code + " </div>" + "<div class='cvk_cell_div3'>" + cell_contents + "</div>" + "</div></td>";
        }
    ...
    key: "_weekStructConv",
    value: function _weekStructConv(calData) {
      var calData2 = [];
      var mw_i = 0;
      var w_i = 0;
    ...
    key: "_mappingToCalData",
    value: function _mappingToCalData(calData, dataA) {
    ...
    key: "_createCalendarDataFormat",
    value: function _createCalendarDataFormat(base_month_start, base_month_end) {
    ...
    key: "_shapingDateInData",
    value: function _shapingDateInData(data, date_field) {
      for (var i in data) {
        var ent = data[i];
    ...
    key: "_getBaseMonthStart",
    value: function _getBaseMonthStart(data, date_field) {
    ...
    key: "_getMonthStartDate",
    value: function _getMonthStartDate(date1) {
    ...
    key: "_getMonthEndDate",
    value: function _getMonthEndDate(date1) {
      if (date1 == null) return null;
    ...
    key: "_aggByDateField",
    value: function _aggByDateField(data, date_field) {
      var dataA = {}; // 日付集計データ
    ...
    key: "_shapingDate",
    value: function _shapingDate(date1) {
      if (date1 == null) return null;
    ...
    key: "_convDateToYMD",
    value: function _convDateToYMD(date1) {
      var year = date1.getFullYear();
      var month = date1.getMonth() + 1;
...
function () {
   ...
   * - 
   */
  function CbBtnSizeChanger(param, cnfData) {
    _classCallCheck(this, CbBtnSizeChanger);

    this.saveKeys = ['cnfData']; // ローカルストレージへ保存と読込を行うparamのキー。

    this.ls_key = "CbBtnSizeChanger"; // ローカルストレージにparamを保存するときのキー。

    this.param = this._setParamIfEmpty(param); // 設定データの初期化

    var cnfData = this._initCnfData(this.param, cnfData); // 設定フォームを作成
    ...
    mainForm.html(cnf_html); // 設定フォームにチェックイベント（クリックイベント）を組み込む

    this._setCheckEvent(mainForm, cnfData); // サブイベントをセットする


    this._setSubEvents(mainForm); // 保存フラグがONであるなら、設定データのボタンサイズ設定を各ボタンへ反映
  ...
  _createClass(CbBtnSizeChanger, [{
    key: "_setParamIfEmpty",
    value: function _setParamIfEmpty(param) {
      if (param == null) param = {}; // ▼ ローカルストレージで保存していたパラメータをセットする
      ...
      if (param['main_slt'] == null) param['main_slt'] = '#CbBtnSizeChanger'; // ラジオボタンデータ
    ...
    key: "_initCnfData",
    value: function _initCnfData(param, cnfDataP) {
    ...
    key: "_getCodeFromSlt",
    value: function _getCodeFromSlt(slt) {
      var code = slt; // コード文字列
      // ▼ 先頭の一文字が「.」または「#」なら除去する。
    ...
    key: "_createCnfFormHtml",
    value: function _createCnfFormHtml(cnfData) {
      // 設定一覧
    ...
    key: "_createRadiosHtml",
    value: function _createRadiosHtml(cnfEnt) {
      var html = ""; // ラジオボタンHTML
      ...
      for (var i in radioData) {
        ...
        var unit_radio_h = "<label class='btn btn-primary btn-sm'><input type='radio' " + "name='" + radio_name + "' " + "value='" + radio_value + "' " + checked_str + " >" + rEnt.wamei + "</label>";
        html += unit_radio_h;
      }
    ...
    key: "_setCheckEvent",
    value: function _setCheckEvent(mainForm, cnfData) {
      var _this2 = this;

      for (var i in cnfData) {
        var cnfEnt = cnfData[i];
        var radio_name = 'cbbsc_r_' + cnfEnt.code;
        mainForm.find("[name='" + radio_name + "']").click(function (e) {
          _this2._checkEvent(e); // ラジオボタンのチェックイベント
    ...
    key: "_checkEvent",
    value: function _checkEvent(e) {
      var _this3 = this;

      var radio = jQuery(e.target);
      var name = radio.attr('name');
      var value = radio.val(); // name属性から設定エンティティを取得する

      var cnfEnt = this._getCnfEntByName(name);

      cnfEnt.size = value; // ▼設定エンティティのセレクタにひもづく要素をループしてサイズを変更する

      jQuery(cnfEnt.slt).each(function (i, btn) {
        _this3._changeSize(btn, value);
      });
    ...
    key: "_getCnfEntByName",
    value: function _getCnfEntByName(name) {
    ...
    key: "_changeSize",
    value: function _changeSize(btn, size) {
      btn = jQuery(btn); // ▼ ボタンサイズのclass属性をいったん除去する
      ...
      if (size == '') return; // サイズ文字列をclass属性に追加する。
    ...
    key: "_changeSizeAll",
    value: function _changeSizeAll(cnfData) {
      var _this4 = this;

      for (var i in cnfData) {
        var cnfEnt = cnfData[i];
        jQuery(cnfEnt.slt).each(function (i, btn) {
          _this4._changeSize(btn, cnfEnt.size);
        });
      }
    }
    /**
    ...
    key: "_setSubEvents",
    value: function _setSubEvents(mainForm) {
      var _this5 = this;

      // ▼「初期に戻す」ボタンにイベントをセットする
      mainForm.find("#cbbsc_def_btn").click(function (e) {
        _this5._returnToInit(); // 初期に戻す

      }); // ▼「閉じる」ボタンにイベントをセットする

      mainForm.find("#cbbsc_close_btn").click(function (e) {
        _this5.mainForm.hide();
      });
    }
    /**
     * 初期に戻す
    ...
    value: function _returnToInit() {
      ...
      this._changeSizeAll(cnfData); // ラジオボタンに設定データを反映する


      this._setCnfDataToRadios(cnfData); // ローカルストレージに設定を保存
    ...
    key: "_setCnfDataToRadios",
    value: function _setCnfDataToRadios(cnfData) {
      for (var i in cnfData) {
        var cnfEnt = cnfData[i];
        var radio_name = 'cbbsc_r_' + cnfEnt.code;
...
function () {
   ...
   * 
   */
  function CbFileUploadComponent(fuIds, option) {
    _classCallCheck(this, CbFileUploadComponent);
  ...
  _createClass(CbFileUploadComponent, [{
    key: "_makeDtData",
    value: function _makeDtData(fields) {
    ...
    key: "_fueIdsToFields",
    value: function _fueIdsToFields(fuIds) {
      var fields = [];
    ...
    key: "_fudIdToField",
    value: function _fudIdToField(fue_id) {
    ...
    */
    ...
    key: "_array_unique",
    value: function _array_unique(ary) {
    ...
    key: "_factoryFileUploadK",
    value: function _factoryFileUploadK(fuIds, option) {
      // 拡張ファイルアップロードクラスの生成
    ...
    key: "addEvent",
    value: function addEvent(fue_id, option) {
    ...
    key: "uploadByAjax",
    value: function uploadByAjax(callback, withData, option) {
    ...
    key: "getFileNames",
    value: function getFileNames(fue_id) {
    ...
    key: "setFilePaths",
    value: function setFilePaths(fue_id, fns, via_dps, option) {
    ...
    key: "_joinDpFn",
    value: function _joinDpFn(dp, via_dp, fn) {
      var fp = ''; // ファイルパス
      // ディレクトリパスの経由ディレクトリパス部分を置換する

      dp = dp.replace('%via_dp', via_dp);
      dp = dp.replace('//', '/'); // ディレクトリパスが空であるならファイル名をファイルパスとして返す。
    ...
    key: "setImageToTr",
    value: function setImageToTr(tr, ent, viaDpFnMap) {
      for (var i in this.fields) {
        var field = this.fields[i]; // TR要素からLabel要素を取得する。
    ...
    key: "setFilesToFd",
    value: function setFilesToFd(fd, form_type) {
    ...
    key: "_getFieldInfoFromFuKey",
    value: function _getFieldInfoFromFuKey(fu_id) {
      // フィールドInfoのデフォルト
      var fieldInfo = {
        'fu_id': fu_id,
        'field': fu_id,
...
var ClmShowHide = function ClmShowHide() {
  /// コールバック関数用
  var my = this; /// プロパティ

  this.props = {}; /// デフォルト列データ

  this.defClmData = {}; // アクティブ列データ
  ...
  this.init = function (tblId, chBoxsId, iniClmData, unique) {
    ...
    var j_clmData = localStorage.getItem(unique + '_clmData'); // ローカルストレージに列JSONが存在しない場合、デフォルトをアクティブ列データにコピーする
    ...
    this.actClmData = actClmData; //列表示チェックボックスを作成

    createClmShowCheckBox_csh(tblId, chBoxsId, actClmData); //列表示チェックボックスのクリックイベント。

    $(".csh_cb").click(function (event) {
      var checked = $(this).prop('checked');
      var index = $(this).attr('index'); //列表示の切替処理
    ...
    for (var i in actClmData) {
      var clm_ent = actClmData[i]; //表示フラグ＝0ならその列を非表示にする。
  ...
  this.getCshAry = function () {
    ...
    var csh_ary = []; //列表示切替チェックボックス群から列データの表示フラグにセットする。
  ...
  this.refresh = function () {
    var tblId = this.props.tblId; //一旦、列をすべて表示する
  ...
  function createClmShowCheckBox_csh(tblId, chBoxsId, clmData) {
    ...
    cbs.append(allCheckedBtn); //「初期に戻す」ボタンを作成
  ...
  function getClmDataFromCheckbox() {
    // チェックボックス群要素のjQueyオブジェクトを取得する
    var chBox = $('#' + my.props.chBoxsId); // アクティブ列データを取得する

    var clmData = my.actClmData; //列表示切替チェックボックス群から列データの表示フラグにセットする。

    for (var i in clmData) {
      // チェックボックス要素を取得する
      var cb = chBox.find("[index=" + i + "]"); // チェック状態を取得し、列データにセットする
...
function () {
   ...
   * - flg
   */
  function CrudBase(param, fieldData) {
    _classCallCheck(this, CrudBase);

    // --- 初期化: CrudBaseBaseクラスのメンバを初期化する ----
    // パラメータに空プロパティがあれば、デフォルト値をセットする
    this.param = this._setParamIfEmpty(param); // フォーム情報の取得と初期化

    this.formInfo = this._initFormInfo(this.param); // フィールドデータにプロパティを追加する

    this.fieldData = this._addMoreFieldData(this.param.tbl_slt, this.fieldData); // フィールドデータへフォーム内の要素情報をセットする
    ...
    this.fieldData = this._setFieldDataFromForm(this.fieldData, this.formInfo, 'delete'); // フィールドハッシュテーブルをフィールドデータから生成する。

    this.fieldHashTable = this._createFieldHashTable(this.fieldData); // デフォルト新規入力エンティティを新規入力フォームから取得する

    this.defNiEnt = this._getEntByForm('new_inp'); // テーブルオブジェクト

    this.tbl = jQuery('#' + this.param.tbl_slt); // 自動保存機能・コンポーネント

    this.autoSave = this._factoryCrudBaseAutoSave(); // 行入替機能・コンポーネント

    this.rowExchange = this._factoryCrudBaseRowExchange(); // 列表示切替機能・コンポーネント

    this.csh = this._factoryClmShowHide(); // ガジェット・コンポーネント（カレンダー、数値スライダー）★★★■■■□□□■■■□□□■■■□□□
    //		this.cbGadgetKj = this._factoryCrudBaseGadgetKj(); 
    // CrudBase・ファイルアップロードコンポーネント

    this.cbFileUploadComp = this._factoryCbFileUploadComponent(); // カレンダー日付ピッカー・ラッパーコンポーネント

    this.datepickerWrap = this._factoryDatepickerWrap(); // ボタンサイズ変更コンポーネント

    this.cbBtnSizeChanger = this._factoryCbBtnSizeChanger(); // カレンダービュー

    this.calendarViewK = this._factoryCalendarViewK(); // CrudBase用リアクティブ機能

    this.crudBaseReact = this._factoryCrudBaseReact();
    this.fueIdCash; // file要素のid属性データ（キャッシュ）
    // テーブル変形

    if (this.param.device_type == 'sp') this.tableTransform(1); // フォームをドラッグ移動およびリサイズできるようにする。

    this._formDragAndResizeSetting(); // エラータイプリストによるエラー処理
    ...
    this._iniRtnSetUrl(); // 検索関連の入力要素にEnterイベントを組み込む
  ...
  _createClass(CrudBase, [{
    ...
    value: function _setParamIfEmpty(param) {
      ...
      if (param['drag_and_resize_flg'] == null) param['drag_and_resize_flg'] = 1; // フォームモード
    ...
    key: "_factoryCrudBaseAutoSave",
    value: function _factoryCrudBaseAutoSave() {
      var autoSave; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(CrudBaseAutoSave);
    ...
    key: "_factoryCrudBaseRowExchange",
    value: function _factoryCrudBaseRowExchange() {
      var _this6 = this;

      var rowExchange; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(CrudBaseRowExchange);
      ...
      rowExchange = new CrudBaseRowExchange(this, null, function () {
        _this6.rowExchangeAfter();
      }); // 行入替機能のボタン表示切替
    ...
    key: "_factoryClmShowHide",
    value: function _factoryClmShowHide() {
      var csh; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(ClmShowHide);
    ...
    key: "_factoryCbFileUploadComponent",
    value: function _factoryCbFileUploadComponent() {
      var cbFileUploadComp; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(CbFileUploadComponent);

      if (t == null || t == 'undefined') {
        // 「空」実装
        cbFileUploadComp = {
          'addEvent': function addEvent() {},
          'getFileParams': function getFileParams() {},
          'uploadByAjax': function uploadByAjax() {},
          'getFileNames': function getFileNames() {},
          'setFilePaths': function setFilePaths() {}
        };
    ...
    key: "_factoryDatepickerWrap",
    value: function _factoryDatepickerWrap() {
      var datepickerWrap; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(DatepickerWrap);

      if (t == null || t == 'undefined') {
        // 「空」実装
        datepickerWrap = {
          'tukishomatu': function tukishomatu() {}
        };
    ...
    key: "_factoryCbBtnSizeChanger",
    value: function _factoryCbBtnSizeChanger() {
      var cbBtnSizeChanger; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(CbBtnSizeChanger);
    ...
    key: "_factoryCalendarViewK",
    value: function _factoryCalendarViewK() {
      var calendarViewK; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(CalendarViewK);
    ...
    key: "_factoryCrudBaseReact",
    value: function _factoryCrudBaseReact() {
      var crudBaseReact; // クラス（JSファイル）がインポートされていない場合、「空」の実装をする。

      var t = _typeof(CrudBaseReact);

      if (t == null || t == 'undefined') {
        // 「空」実装
        crudBaseReact = {
          'init': function init() {},
          'reactivatingOfRow': function reactivatingOfRow() {}
        };
    ...
    key: "_getFueIds",
    value: function _getFueIds(form_type, option) {
      if (option == null) option = {};
      if (option['cash_flg'] == null) option['cash_flg'] = 1; // キャッシュがあるならそれを返す。
    ...
    key: "newInpShow",
    value: function newInpShow(elm, option, callBack) {
      if (option == null) option = {}; // 新規行を上に追加するか、下に追加するかを決定する。
      ...
      this.setValueToFrom(form, 'form_type', 'new_inp'); // バリデーションエラーメッセージをクリアする

      this._clearValidErr(form); // コールバックを実行する
      ...
      if (this.param.form_mode == 1) {
        ...
        form = this._setColspanToForm(form); // tdのcolspan属性に表示中の列数をセットする

        var td = form.find('td'); // ▼ SPモードの場合はblock, PCモードの場合はtable系にする。
    ...
    key: "editShow",
    value: function editShow(elm, option, callBack) {
      if (option == null) option = {};
      ...
      var ent = this.getEntityByTr(tr); // フォームに親要素内の各フィールド値をセットする。

      this.setFieldsToForm('edit', ent, option); // 編集フォーム要素を取得

      var form = this.getForm('edit');
      form.show(); // バリデーションエラーメッセージをクリアする
      ...
      if (this.param.form_mode == 1) {
        // フォームのcolspan属性に列数をセットする。
        form = this._setColspanToForm(form); // TR要素の下に編集フォームを移動する
    ...
    key: "copyShow",
    value: function copyShow(elm, option, callBack) {
      var tr = jQuery(elm).parents('tr'); // 先祖をさかのぼりtr要素を取得する
      ...
      var form = this.getForm('copy'); // フォーム種別をフォーム内の指定入力要素へセット

      this.setValueToFrom(form, 'form_type', 'copy'); // 行番を取得し、要素にセットする

      var row_index = tr.index(); // 行番（インデックス）を取得する

      this.setValueToFrom(form, 'row_index', row_index); // フォームにエンティティの値をセットする

      this.setFieldsToForm('new_inp', ent, option); // バリデーションエラーメッセージをクリアする
    ...
    key: "deleteAction",
    value: function deleteAction(elm, option) {
    ...
    key: "enabledAction",
    value: function enabledAction(elm, option) {
      if (!(elm instanceof jQuery)) elm = jQuery(elm);
      if (option == null) option = {};
      var tr = elm.parents('tr'); // 先祖をさかのぼりtr要素を取得する
    ...
    key: "eliminateShow",
    value: function eliminateShow(elm, option, callBack) {
      if (!(elm instanceof jQuery)) elm = jQuery(elm);
      var tr = elm.parents('tr'); // 先祖をさかのぼりtr要素を取得する
      ...
      this.setFieldsToForm('eliminate', ent, option); // コールバックを実行する

      if (callBack) callBack(tr, form, ent); // triggerElm要素の下付近に入力フォームを表示する。
    ...
    key: "newInpReg",
    value: function newInpReg(beforeCallBack, afterCallBack, option) {
      var _this7 = this;
      ...
      if (res == false) return;
      if (this._empty(option)) option = {}; // 登録ボタンを押せなくする

      this._toggleRegBtn('new_inp', 0); // FDにファイルオブジェクトをセットする。


      var fd = new FormData();
      fd = this.cbFileUploadComp.setFilesToFd(fd, 'new_inp'); // 新規入力フォームからエンティティを取得およびJSON化し、FormDataにセットする

      var ent = this._getEntByForm('new_inp'); // idに値がセットされ編集扱いとなっている場合、IDフラグをtrueにする。
      ...
      fd.append('reg_param_json', reg_param_json); // 諸パラメータから追加行インデックスを決定する
      ...
      jQuery.ajax({
        ...
        _this7._toggleRegBtn('new_inp', 1); // 登録ボタンを押せるようにする
        ...
        if (ent['err']) {
          // エラーをフォームに表示する
          _this7._showErrToForm(ent['err'], 'new_inp');
        } else {
          ...
          _this7.closeForm('new_inp'); // フォームを閉じる
          // 新しい行を作成する


          var tr = _this7._addTr(ent, add_row_index); // ボタン群の表示切替


          _this7._switchBtnsDisplay(tr, ent); // 登録後にコールバック関数を非同期で実行する
    ...
    key: "editReg",
    value: function editReg(beforeCallBack, afterCallBack, option) {
      var _this8 = this;
      ...
      this._toggleRegBtn('edit', 0); // 編集フォームからエンティティを取得する。


      var ent = this._getEntByForm('edit'); // FDにファイルオブジェクトをセットする。


      var fd = new FormData();
      fd = this.cbFileUploadComp.setFilesToFd(fd, 'edit'); // Ajax送信前のコールバックを実行する
      ...
      fd.append("key1", json); // WordPressの場合
      ...
      jQuery.ajax({
        ...
        _this8._toggleRegBtn('edit', 1); // 登録ボタンを押せるようにする
        ...
        if (ent) {
          if (ent['err']) {
            // エラーをフォームに表示する
            _this8._showErrToForm(ent['err'], 'edit');

            return;
          } // 編集中のTR要素を取得する


          var tr = _this8._getTrInEditing(); // 削除フラグがONである場合、削除中の行を一覧から隠す


          if (_this8.param['kj_delete_flg'] == 0 && ent['delete_flg'] == 1) {
            tr.hide();
          ...
          _this8._setEntityToEditTr(ent, tr); // 登録後にコールバック関数を非同期で実行する
          ...
          _this8.closeForm('edit'); // フォームを閉じる
    ...
    key: "deleteRegByRowIndex",
    value: function deleteRegByRowIndex(row_index, option) {
      // HTMLテーブルから行番を指定してエンティティを取得する
      var ent = this.getEntity(row_index); // 削除を実行
    ...
    key: "eliminateReg",
    value: function eliminateReg(option) {
      if (option == null) option = {};
    ...
    key: "closeForm",
    value: function closeForm(form_type) {
      // フォーム情報を取得
      var fi = this.formInfo[form_type]; // フォームのオブジェクトを取得する

      var form = fi.form;
      jQuery('#crud_base_forms').append(form); // フォームを隠す
    ...
    key: "rowExchangeShowForm",
    value: function rowExchangeShowForm(btnElm) {
    ...
    key: "rowExcBtnShow",
    value: function rowExcBtnShow(row_exc_flg) {
      if (row_exc_flg == 1) {
    ...
    key: "resetKjs",
    value: function resetKjs(exempts) {
      if (exempts == null) exempts = []; //デフォルト検索条件JSONを取得およびパースする。
    ...
    value: function getContainer() {
      var container = {
        ...
        //  フィールドデータ
        'fieldHashTable': this.fieldHashTable,
        //  フィールドハッシュテーブル key:フィールド名  val:列インデックス
        'formInfo': this.formInfo,
        //  フォーム情報
        'defNiEnt': this.defNiEnt,
        //  デフォルト新規入力エンティティ
        'formNewInp': this.formNewInp,
        // 新規入力フォーム
        'formEdit': this.formEdit,
        // 編集フォーム
        'formDelete': this.formDelete,
        // 削除フォーム
        'showFormStrategy': this.showFormStrategy,
        //  入力フォーム表示ストラテジー
    ...
    key: "_decAddRowIndex",
    value: function _decAddRowIndex(form, form_type, option) {
    ...
    key: "deleteShow",
    value: function deleteShow(elm, option) {
      if (!(elm instanceof jQuery)) elm = jQuery(elm);
      var tr = option['tr'];
      ...
      this.setFieldsToForm('delete', ent, option); // コールバックを実行する
    ...
    key: "deleteReg",
    value: function deleteReg(option) {
      if (option == null) option = {};
    ...
    key: "enabledReg",
    value: function enabledReg(option) {
      if (option == null) option = {};
      var ent = this.getEntity(); // 有効を実行
    ...
    key: "_deleteRegBase",
    value: function _deleteRegBase(ent, delete_flg, option) {
      var _this9 = this;

      if (!ent['id']) throw new Error('Not id'); // 抹消フラグ

      var eliminate_flg = 0;
      if (option['eliminate_flg']) eliminate_flg = option['eliminate_flg']; // フォーム種別を取得する

      var form_type = 'delete';
      if (eliminate_flg) form_type = 'eliminate';
      ent['delete_flg'] = delete_flg;
      if (option == null) option = {}; // ファイルアップロード関連のエンティティをFormDataに追加する

      var fd = new FormData(); // 登録パラメータに各種値をセット
      ...
      var reg_param_json = JSON.stringify(regParam);
      fd.append('reg_param_json', reg_param_json); // Ajax送信前のコールバックを実行する
      ...
      jQuery.ajax({
        ...
        if (_this9.param.kjs.kj_delete_flg === 0 || _this9.param.kjs.kj_delete_flg === '0' || eliminate_flg == 1) {
          _this9.activeTr.remove(); // アクティブ行を削除

        } else {
          var tr = _this9._getTrInEditing(); // 削除中の行要素を取得する


          _this9._setEntityToEditTr(ent, tr); // TR要素にエンティティの値をセットする
        ...
        _this9.closeForm(form_type); // フォームを閉じる
    ...
    key: "getTr",
    value: function getTr(row_index) {
    ...
    key: "_getTrInEditing",
    value: function _getTrInEditing() {
    ...
    key: "getTd",
    value: function getTd(row_index, field) {
    ...
    key: "getTdInEditing",
    value: function getTdInEditing(field) {
    ...
    key: "getEntity",
    value: function getEntity(row_index) {
      // 行番を指定してTR要素を取得する
      var tr = this.getTr(row_index); // TR要素からエンティティを取得する
    ...
    key: "getEntityByTr",
    value: function getEntityByTr(tr) {
      var ent = {};
     ...
     * @return エンティティ
     */
    value: function getEntityByInnerElm(elm) {
      // 先祖をさかのぼりtr要素を取得する
      var tr = jQuery(elm).parents('tr'); // 行番（インデックス）を取得する

      var index = tr.index(); // 一覧行から行番にひもづくエンティティを取得する
     ...
     * @return object データ
     */
    value: function getDataFromTbl(tbl) {
      var _this10 = this;
      ...
      var data = []; // テーブルの行をループする

      trs.each(function (i, elm) {
        var tr = jQuery(elm); // TR要素からエンティティを取得する

        var ent = _this10.getEntityByTr(tr);
    ...
    key: "getDataFromTblByFields",

    /**
     * フィールドリストを指定して、Htmlテーブルからデータを取得する
     * @return object データ
     */
    value: function getDataFromTblByFields(fields) {
      var _this11 = this;

      var slt = '#' + this.param.tbl_slt + ' tbody tr';
      var data = []; // テーブルの行をループする

      jQuery(slt).each(function (i, elm) {
        var tr = jQuery(elm); // TR要素からフィールド名を検索し、値を取得する
     ...
     * @returns 要素の値
     */
    value: function _getValueEx(elm) {
    ...
    value: function getValueFromForm(form, field) {
    ...
    key: "setValueToFrom",
    value: function setValueToFrom(form, field, value) {
      var inpElm = this._formFind(form, field); // 入力要素を取得
    ...
    key: "_showErrToForm",
    value: function _showErrToForm(err, form_type) {
      ...
      var form = jQuery(info.slt); // フォーム要素からエラー要素を取得

      var errElm = form.find('.err'); // エラー要素にエラーメッセージを埋め込む。
    ...
    key: "_clearValidErr",
    value: function _clearValidErr(form) {
    ...
    key: "_validationCheckForm",
    value: function _validationCheckForm(form_type) {
      var _this12 = this;
      ...
      form.find('.valid').each(function (i, elm) {
        var elm = jQuery(elm);

        var field = _this12._getFieldByNameOrClass(elm); // 入力要素単位でバリデーションを行う


        var res = _this12._validationCheck(elm, field);
     ...
     * @return validFlg バリデーションフラグ true:正常 false:入力エラー
     */
    value: function _validationCheck(elm, field) {
      var validFlg = true; // バリデーションフラグ
    ...
    key: "_resetFileUpload",
    value: function _resetFileUpload(fuEnt) {
      fuEnt.evt = null;
    ...
    key: "_addTr",
    value: function _addTr(ent, add_row_index, option) {
      if (add_row_index == null) add_row_index = -1;
      add_row_index *= 1; // 数値型に変換する。

      if (!option) option = {}; // テーブルのtbody要素を取得する
      ...
      var new_tr_html = "<tr style='background-color:#fdeaea'>" + tr0.html() + "</tr>"; // TR要素をテーブルの指定行に挿入する

      var newTr = this._insertTr(tbl_slt, add_row_index, new_tr_html); // 新行要素にエンティティをセットする
    ...
    key: "_insertTr",
    value: function _insertTr(tbl_slt, row_index, tr_html) {
      var tbody = jQuery(tbl_slt + " tbody");
      var trs = tbody.find("tr");
      var tr_len = trs.length;
      var new_index = null; // 行数が1件以上である場合

      if (tr_len > 0) {
        // 追加行番が行数未満である場合
        if (row_index < tr_len && row_index > -1) {
          // 行番にひもづくTR要素を取得
          var tr = trs[row_index]; // TR要素の上に新TR要素を追加
          ...
          new_index = row_index;
            ...
            // 最後のTR要素を取得
            var tr = trs[tr_len - 1]; // TR要素の下に新TR要素を追加する
    ...
    key: "_setEntityToEditTr",
    value: function _setEntityToEditTr(ent, tr, option) {
      if (ent == null) return;
      if (!option) option = {}; // 現在編集中の行要素を取得する
      ...
      this.setEntityToTr(tr, ent, option); // ボタン群の表示切替

      this._switchBtnsDisplay(tr, ent);
    }
  }, {
    key: "_switchBtnsDisplay",
     ...
     * @param ent 行エンティティ
     */
    value: function _switchBtnsDisplay(tr, ent) {
      // 削除状態のボタン表示切替
      if (ent['delete_flg'] == 1) {
    ...
    key: "_switchBtnDisp",
    value: function _switchBtnDisp(tr, btn_slt, show_flg) {
    ...
    key: "setDataToTbl",
    value: function setDataToTbl(tbl, data, option) {
      var _this13 = this;
      ...
      trs.each(function (i, tr) {
        ...
        _this13.setEntityToTr(tr, ent, option); // TR要素にエンティティをセットする
    ...
    key: "setEntityToTr",
    value: function setEntityToTr(tr, ent, option) {
      if (ent == null) return;
      if (option == null) option = {}; // XSSサニタイズ
    ...
    key: "_getEntByForm",
    value: function _getEntByForm(form_type) {
      var _this14 = this;

      // 現在編集中の行要素を取得する
      var tr = this._getTrInEditing(); // TR要素からエンティティを取得する
      ...
      for (var i in this.fieldData) {
        ...
        var f = this.fieldData[i].field; // name属性またはclass属性を指定して入力要素を取得する。

        var inps = this._formFind(form, f); // 該当する入力要素の件数を取得する
          ...
          continue;
            ...
            v = this._getEntByForm2(inps, form, f);
              ...
              inps.each(function (i, elm) {
                var inp = jQuery(elm);
                v = _this14._getEntByForm2(inp, form, f);
                return;
              });
            }
    ...
    key: "getForm",
    value: function getForm(form_type, cache) {
    ...
    key: "_getEntByForm2",
    value: function _getEntByForm2(inp, form, f) {
    ...
    key: "_getValFromLabel",
    value: function _getValFromLabel(form, field) {
      var v = null;
      var label = form.find("[for='" + field + "']");
    ...
    key: "_formFind",
    value: function _formFind(form, feild) {
    ...
    key: "_convertFormToDlg",
    value: function _convertFormToDlg(formInfo, form_z_index) {
      ...
      var form = formInfo.form; //デフォルトCSSデータ

      var cssData = {
        'z-index': form_z_index,
        'position': 'absolute' // フォームにCSSデータをセットする

      };
      form.css(cssData); //ツールチップの外をクリックするとツールチップを閉じる
    ...
    key: "_setFieldDataFromForm",
    value: function _setFieldDataFromForm(fieldData, formInfo, form_type) {
      // フォーム要素オブジェクトを取得する
      ...
      for (var i in fieldData) {
        ...
        inp_ent['elm'] = inp; // 入力要素が取得できなければcontinueする。
        ...
        inp_ent['tag_name'] = tag_name; // 値を取得する
    ...
    key: "trToDiv",
    value: function trToDiv(par, row_index, option) {
    ...
    key: "entToBinds",
    value: function entToBinds(par, ent, bind_attr, option) {
      var bindElms = this.getBindElms(par, bind_attr, option); // class属性に紐づくバインド要素リストを親要素から取得する

      if (!option) option = {}; // class属性である場合、表示フィルターをONにする。name属性であるならOFFにする。
    ...
    key: "getBindElms",
    value: function getBindElms(par, bind_attr) {
      var bindElms = {};
      ...
      for (var i in this.fieldData) {
        var fEnt = this.fieldData[i]; //要素を取得する
        ...
        if (bind_attr == 'class') {
          ...
          bElms = par.find("[name='".concat(fEnt.field, "']"));
        } else {
          continue;
        }
    ...
    key: "setFieldsToForm",
    value: function setFieldsToForm(form_type, ent, option) {
    ...
    value: function setValueToElement(elm, field, val1, ent, option) {
      if (!(elm instanceof jQuery)) elm = jQuery(elm); // 要素がjQueryオブジェクトでなければ、jQueryオブジェクトに変換。
      // オプションの初期化

      if (option == null) option = {}; // サニタイズフラグを取得する
      ...
      if (tag_name == 'INPUT' || tag_name == 'SELECT') {
        ...
        if (typ == 'file') {
          ...
          if (typeof val1 == 'string') {
            val1 = val1.replace(/<br>/g, "\r"); // XSSサニタイズを施す
    ...
    key: "displayFilter",
    value: function displayFilter(val1, field, disFilData, xss) {
      var res = {
        'val1': val1,
        'xss': xss
      };
    ...
    key: "setDisplayFilterData",
    value: function setDisplayFilterData(disFilData) {
    ...
    key: "disFilSelect",
    value: function disFilSelect(val1, field, option) {
    ...
    key: "disFilDeleteFlg",
    value: function disFilDeleteFlg(val1, field, option) {
      if (val1 == null) return val1;
    ...
    key: "disFilFlg",
    value: function disFilFlg(val1, field, option) {
    ...
    key: "disFilMoney",
    value: function disFilMoney(val1, field, option) {
      if (val1 == null) return val1;
      var currency = "&yen;";
    ...
    key: "_setToFormForFile",
    value: function _setToFormForFile(form_type, form, field, v, ent, option) {
    ...
    key: "_getFieldEntByField",
    value: function _getFieldEntByField(field) {
    ...
    key: "_showForm",
    value: function _showForm(form, triggerElm, option) {
      if (option == null) option = {}; // フォーム位置フラグをセットする。SP(スマホ）である場合は、max(横幅いっぱい）とする。
      ...
      if (this.param.auto_close_flg == 0) {
        this.param.form_z_index++; // ※ z_indexの最大値は32bit整数,あるいは64bit整数の最大値である。

        form.css('z-index', this.param.form_z_index);
      }

      form.show(); // 入力フォームストラテジーがセットされている場合、こちらの処理を実行する。（ストラテジーパターンによる拡張である）
    ...
    key: "_addMoreFieldData",
    value: function _addMoreFieldData(tbl_slt, fieldData) {
      ...
      var fieldData2 = []; // thループ

      var i = 0;
      jQuery('#' + tbl_slt + ' th').each(function (i, elm) {
    ...
    key: "_getFieldDataFromTh",
    value: function _getFieldDataFromTh() {
      var _this15 = this;
      ...
      jQuery(slt).each(function (i, elm) {
        ...
        var field = _this15._getFieldByClassOrName(thElm); ////class属性またはname属性からフィールド名を取得する
    ...
    key: "_createFieldHashTable",
    value: function _createFieldHashTable(fieldData) {
      var hashTable = {};
    ...
    key: "_initFormInfo",
    value: function _initFormInfo(param) {
      var formInfo = {}; // フォーム情報
      // 新規フォーム情報の設定
     ...
     * @returns void
     */
    value: function _errShow(errData, form_type) {
      // フォーム種別からフォーム要素を取得する
      var form = this.getForm(form_type); // 一旦、バリデーションエラーメッセージをクリアする

      this._clearValidErr(form); // エラーメッセージのみである場合
    ...
    value: function searchKjs() {
      ...
      formElm.find('.kjs_inp').each(function (i, elm) {
        // 要素から選択値を取得する
        var kjsElm = jQuery(elm);
        var val = kjsElm.val(); // 選択値が空でない場合（0も空ではない扱い）

        var field = kjsElm.attr('id'); // 要素のID属性から検索条件フィールドを取得する

        if (!_this16._emptyNotZero(val)) {
      ...
      jQuery.extend(param, kjs); // パラメータから除去リストのフィールドを削除する。
      ...
      param['page_no'] = 0; // パラメータからURLクエリを組み立てる
    ...
    key: "isDefaultKjs",
    value: function isDefaultKjs(outFields, kjs, defKjs) {
    ...
    */
    ...
    key: "_getFieldByNameOrClass",
    value: function _getFieldByNameOrClass(elm) {
    ...
    */
    ...
    key: "_getFieldByClassOrName",
    value: function _getFieldByClassOrName(elm) {
    ...
    key: "_classifySlt",
    value: function _classifySlt(slt) {
      var xid = '';
      var slt2 = '';
    ...
    key: "_xssSanitaizeEncode",
    value: function _xssSanitaizeEncode(data) {
      if (_typeof(data) == 'object') {
        for (var i in data) {
    ...
    key: "_xssSanitaizeDecode",
    value: function _xssSanitaizeDecode(data) {
      if (_typeof(data) == 'object') {
        for (var i in data) {
    ...
    key: "_emptyNotZero",
    value: function _emptyNotZero(v) {
    ...
    key: "setShowFormStrategy",
    value: function setShowFormStrategy(obj) {
    ...
    key: "_setValueEx",
    value: function _setValueEx(elm, v) {
      if (!elm[0]) {
        return;
      }
    ...
    key: "saveRequest",
    value: function saveRequest(data, option) {
    ...
    key: "escapeForAjax",
    value: function escapeForAjax(data) {
    ...
    key: "rowExchangeAfter",
    value: function rowExchangeAfter() {
      var data = this.getDataFromTbl(); // Htmlテーブルからデータを取得

      var page_no = jQuery("#page_no").val() * 1;
      var limit = jQuery("#row_limit").val() * 1; // データに順番をセットする
    ...
    key: "_toggleRegBtn",
    value: function _toggleRegBtn(form_type, active_flg) {
      var form = this.getForm(form_type); // フォーム要素を取得
    ...
    key: "_snakeToCamel",
    value: function _snakeToCamel(str) {
     ...
     * 検索条件入力要素リストを取得する
     */
    value: function getKjElms() {
      var kjElms = {};
      jQuery('.kj_wrap').each(function (i, elm) {
        elm = jQuery(elm);
    ...
    key: "tableTransform",
    value: function tableTransform(mode_no) {
      if (mode_no == 1) {
    ...
    key: "_fukPrevAfterCallback",
    value: function _fukPrevAfterCallback(fue_id, box, cbParam) {
      if (fue_id == null) return; // fue_idの末尾の2文字からフォーム種別を判定する。
    ...
    key: "_fitFormByFormType",
    value: function _fitFormByFormType(form_type) {
      var form = this.getForm(form_type);
    ...
    key: "_formDragAndResizeSetting",
    value: function _formDragAndResizeSetting() {
      if (this.param.form_mode == 0) {
    ...
    key: "_formDragAndResizeSetting2",
    value: function _formDragAndResizeSetting2(form_type) {
      if (this.param['drag_and_resize_flg'] != 1) return;
      var form = this.getForm(form_type); // フォームのドラッグ移動を有効にする。

      var draggableDiv = form.draggable(); //ドラッグ移動を組み込むとテキスト選択ができなくなるので、パネルボディ部分をテキスト選択可能にする。
    ...
    key: "openNoteDetail",
    value: function openNoteDetail(btnElm, field) {
      if (field == null) field = 'note'; // 親要素であるTD要素を取得する

      var td = btnElm.parents('td'); // フィールド要素を取得する

      var fieldElm = td.find("[name='" + field + "']"); // 短文要素を取得する

      var shortElm = td.find('.' + field); // ノート詳細要素を作成および取得する
      ...
      if (text1 == null || text1 == '') return; // XSSサニタイズ

      text1 = this._xssSanitaizeEncode(text1); // 改行コードをBRタグに変換する

      text1 = text1.replace(/\r\n|\n\r|\r|\n/g, '<br>'); // ノート詳細にテキストをセットする

      noteDetailElm.html(text1); // 短文要素を隠し、詳細ボタン名も変更する
    ...
    key: "_setColspanToForm",
    value: function _setColspanToForm(form) {
      // 現在表示中の列数を取得する
      var clm_cnt = this._getClmCntByActive(); // フォーム内のTD要素・colspan属性に列数をセットする。
    ...
    key: "_getClmCntByActive",
    value: function _getClmCntByActive() {
      // 表示中の列数を取得する
    ...
    key: "_setColspanToTd",
    value: function _setColspanToTd(td) {
      // 表示中の列数を取得する
      var cshAry = this.csh.getCshAry();
      var clm_cnt = 1; // 列数 (最初の1はボタン類の列を表す）
    ...
    key: "_stringLeft",
    value: function _stringLeft(s, mark, not_find_flg) {
    ...
    key: "_addEventForKjsEnter",
    value: function _addEventForKjsEnter() {
      var _this17 = this;
      ...
      formKjsElm.find('input').keypress(function (e) {
        if (e.which == 13) {
          // Enterキーが押下された場合
          _this17.searchKjs(); // 検索実行
    ...
    key: "calendarViewCreate",
    value: function calendarViewCreate(date_field) {
      // 一覧テーブルからデータを取得する
      var data = this.getDataFromTbl(); // カレンダービューを生成
    ...
    key: "reactInit",
    value: function reactInit(hyo_elm_id) {
      // ■■■□□□■■■□□□■■■□□□
      var d1 = new Date();
      var t1 = d1.getTime(); //		console.log('this.fieldData');//■■■□□□■■■□□□■■■□□□)
    ...
    key: "reactivatingOfRow",
    value: function reactivatingOfRow() {
...
function () {
   ...
   */
  function CrudBaseAutoSave(crudBase) {
    _classCallCheck(this, CrudBaseAutoSave);
  ...
  _createClass(CrudBaseAutoSave, [{
    ...
    value: function saveRequest(data, option) {
      ...
      if (option['interval'] == null) option['interval'] = 3000; // setTimeoutの処理を一旦キャンセルする。
      ...
      this.set_timeout_hdl = setTimeout(function () {
        _this18._autoSave(_this18.data); // 自動保存


        if (option['reflect_on_tbl'] == 1) {
          _this18.crudBase.setDataToTbl(null, data); // データをHTMLテーブルに再セットする
    ...
    value: function _autoSave(data) {
      ...
      jQuery.ajax({
        ...
        try {
          ...
          _this19.msgElm.html('自動保存のエラー1');

          jQuery("#err").html(str_json);
          return;
        }
      }).fail(function (jqXHR, statusText, errorThrown) {
        _this19.msgElm.html('自動保存のエラー2');
...
function () {
   ...
   * - flg
   */
  function CrudBaseReact(param) {
    _classCallCheck(this, CrudBaseReact);
  }
  /**
   * If Param property is empty, set a value.
   */
  ...
  _createClass(CrudBaseReact, [{
    ...
    key: "init",
    value: function init(fields, hyo_elm_id) {
      this.fields = fields; // 表IDリストの取得とトリミング
...
function () {
   ...
   */
  function CrudBaseRowExchange(crudBase, param, afterCallback) {
    var _this20 = this;

    _classCallCheck(this, CrudBaseRowExchange);
    ...
    param['form_slt'] = form_slt; // 行入替フォームのHTML文字列を取得する

    var formHtml = this._getFormHtml(form_slt); // テーブル要素の下に行入替フォームを追加、およびオブジェクトを取得


    this.tbl.after(formHtml);
    this.form = jQuery(form_slt); // 行入替のフォーム

    this.param = param; // 閉じるボタンのクリックイベント
    ...
    jQuery(form_slt + ' input').keypress(function (e) {
      if (e.which == 13) {
        // Enterキーである場合
        _this20._exchageTrReb(); // 行入替ボタンによる行入替処理

      }
    }); // 上シフトボタンのクリックイベント

    this.form.find('.exchange_tr_shift_up').click(function () {
      _this20._exchageTrShiftUp(); // 上シフトボタンによる行入替処理

    }); // 下シフトボタンのクリックイベント

    this.form.find('.exchange_tr_shift_down').click(function () {
      _this20._exchageTrShiftDown(); // 下シフトボタンによる行入替処理
  ...
  _createClass(CrudBaseRowExchange, [{
    key: "showForm",
    value: function showForm(btn) {
      ...
      this.param['from_row_index'] = from_row_index; // 移動先テキストボックスが空であるなら、上記で取得した移動元インデックスを初期セットする。
    ...
    key: "_exchageTrReb",
    value: function _exchageTrReb() {
      // 移動元行インデックスを取得する
      var from_row_index = this.param['from_row_index']; // 移動先行インデックスを取得する

      var to_row_index = this.form.find('.exchange_tr_sort_no').val(); // 行インデックスをチェックし、不正があれば、処理中断
    ...
    key: "_exchageTrShiftUp",
    value: function _exchageTrShiftUp() {
      // 移動元行インデックスを取得する
      var from_row_index = this.param['from_row_index']; // 移動先行インデックスを取得する

      var to_row_index = from_row_index - 1; // 移動先行番が0以下なら上シフト処理を中断する
      ...
      this._exchageTr(from_row_index, to_row_index); // 連続して上シフトボタンを押した時のために、移動先を移動元として保存しておく
    ...
    key: "_exchageTrShiftDown",
    value: function _exchageTrShiftDown() {
      // 移動元行インデックスを取得する
      var from_row_index = this.param['from_row_index']; // 移動先行インデックスを取得する

      var to_row_index = from_row_index + 1; // テーブルの行数を取得する
    ...
    key: "_exchageTr",
    value: function _exchageTr(from_row_index, to_row_index) {
      // 移動元と移動先のTR要素を取得する
      var tr1 = this.tbl.find('tr').eq(from_row_index);
      var tr2 = this.tbl.find('tr').eq(to_row_index); // 移動元と移動先のTR要素を入れ替える
    ...
    key: "_checkRowIndex",
    value: function _checkRowIndex(row_index) {
    ...
    key: "_showForm",
    value: function _showForm(form, triggerElm, form_position) {
      if (!form_position) {
        form_position = 'auto';
      }

      form.show(); //トリガー要素の右上位置を取得
...
function () {
   ...
   */
  function DatepickerWrap(param) {
    _classCallCheck(this, DatepickerWrap);

    this.param = this._setParamIfEmpty(param); // カレンダーを日本語化する
  ...
  _createClass(DatepickerWrap, [{
    ...
    key: "_datepickerApply",
    value: function _datepickerApply() {
    ...
    key: "_datetimepickerApply",
    value: function _datetimepickerApply() {
      if (this.param.datetimepicker_f == 0) return;
      jQuery('.datetimepicker').each(function (i, elm) {
    ...
    value: function _ympickerApply() {
      ...
      jQuery('.ympicker').each(function (i, elm) {
    ...
    key: "tukishomatu",
    value: function tukishomatu(tb_ym_id, tb_m_start_id, tb_m_ent_id) {
      //年月入力
      var op = {
        closeText: '閉じる',
        prevText: '&#x3c;前',
        ...
        onSelect: function onSelect(date, instance) {
          //月初日の作成と出力
          var d1 = date + '/1';
          $('#' + tb_m_start_id).val(d1); //月末日の算出と出力
      ...
      $('#' + tb_ym_id).ympicker(op); //年月をクリアしたら月初、月末もクリアする。
...
function () {
   ...
   */
  function FileUploadK(param) {
    _classCallCheck(this, FileUploadK);
  ...
  _createClass(FileUploadK, [{
    ...
    value: function _setParamIfEmpty(param) {
      ...
      if (param['img_height'] == null) param['img_height'] = null; // 補足データフラグリスト
    ...
    value: function addEvent(fue_id, option) {
      ...
      if (option == null) option = {}; // ファイルアップロード要素の親ラベル（DnD要素）を取得する

      var parLabel = this._getElement(fue_id, 'label'); // 親ラベル要素にfue_idを属性として追加する。


      parLabel.attr('data-fue-id', fue_id); // 各要素から値を取ってきて、データボックスに格納する。

      var box = this._setToBox(fue_id, this.box, option); // 親ラベル要素にいくつかの関連要素を追加する。


      this._addRelatedElements(parLabel, fue_id, box); // 関連要素にCSSスタイルを適用する


      this._applyStyle(fue_id); // DnDイベントをラッパー要素に追加


      parLabel[0].addEventListener('drop', function (evt) {
        evt.stopPropagation();
        evt.preventDefault(); // ボックスデータにアップロードファイル情報を追加

        var files = evt.dataTransfer.files;
        _this21.box[fue_id]['files'] = files;

        _this21._preview(fue_id, 'files', option); // プレビュー表示

      }, false); // ドラッグオーバーイベントを発動させないようにする。

      parLabel[0].addEventListener('dragover', function (evt) {
        evt.preventDefault();
      }, false); // ファイルアップロード要素のチェンジイベントを追加する。

      var fue = this._getElement(fue_id, 'fue');

      fue.change(function (e) {
    ...
    key: "setFilePaths",
    value: function setFilePaths(fue_id, fps, option) {
      var _this22 = this;
      ...
      option['pacb'] = function () {
        //複数非同期・全終了後コールバック
        // プレビュー表示
        _this22.box[fue_id]['bData'] = bData;

        _this22._preview(fue_id, 'blob', option);
      }; // 複数非同期・全終了後コールバック・初期化


      this._cbAsynsEndInit(fps.length, option); // ファイルをXHRでプリロードする
    ...
    key: "_preloadByXhr",
    value: function _preloadByXhr(fp, bData) {
      var _this23 = this;
      ...
      xhr.onload = function (e) {
        ...
        _this23._xhrOnload(e, xhr, bData);
      };
    ...
    key: "_xhrOnload",
    value: function _xhrOnload(e, xhr, bData) {
      // Blobを取得する
      ...
      bData.push(bEnt); // 複数非同期・全終了後コールバック・アクション
    ...
    key: "_addRelatedElements",
    value: function _addRelatedElements(parLabel, fue_id, box) {
      var html = ''; // 初期メッセージ要素を追加

      var fuk_msg_text = box[fue_id]['fuk_msg_text']; // 初期メッセージテキスト

      html += "<span class='fuk_msg'>" + fuk_msg_text + "</span>"; // プレビュー要素を追加

      html += "<span class='fuk_preview' style='display:inline-block'></span>"; // クリアボタン用を追加

      html += "<div class='fuk_clear_btn_w'><input type='button' value='Clear' class='btn btn-default btn-xs fuk_clear_btn' " + "data-fue-id='" + fue_id + "' /></div>";
    ...
    key: "_applyStyle",
    value: function _applyStyle(fue_id) {
    ...
    key: "_preview",
    value: function _preview(fue_id, bin_type, option) {
      var fukMsg = this._getElement(fue_id, 'fuk_msg');
      ...
      if (prvElm[0]) prvElm.html(preview_html); // リソースプレビュー要素を取得する。リソースプレビュー要素はプレビュー区分要素内に複数存在するDIV要素群のこと。

      var rpElms = {}; // リソースプレビュー要素リスト

      prvElm.children('div').each(function (i, elm) {
        elm = jQuery(elm);
      ...
      if (bin_type == 'files') {
        ...
        this._cbAsynsEndInit(fileData.length, option); // リソース（画像など）をリソースプレビュー要素に表示させる。（非同期処理あり）


        for (var i in fileData) {
          var fEnt = fileData[i];
          var index = fEnt.index;
          var file = files[index]; // リソース（ファイル）のレンダリングを行い、リソースプレビュー要素に画像などを表示する。
    ...
    key: "_checkFnsEmpty",
    value: function _checkFnsEmpty(fileData) {
      for (var i in fileData) {
        var fEnt = fileData[i];
    ...
    key: "_filteringFileData",
    value: function _filteringFileData(fileData, rpElms) {
      var fileData2 = [];
    ...
    key: "_setupRender",
    value: function _setupRender(file, rpElms, i) {
      var _this24 = this;

      var reader = new FileReader();
      reader.readAsDataURL(file); // After conversion of the event.

      reader.onload = function (evt) {
        var rpElm = rpElms[i];
        rpElm.attr('src', reader.result);

        _this24._cbAsynsEndAction('exe1'); // プレビュー後コールバックアクション：制御と実行1
    ...
    key: "_getFileDataFromFiles",
    value: function _getFileDataFromFiles(files) {
      ...
      for (var i in files) {
        ...
        var mime = file.type;
        fEnt['mime'] = mime; // MIMEからファイル種別を判別する。
        ...
        fEnt['file_type'] = file_type; // ファイル名

        var fn = '';
        if (file.name != null) fn = file.name;
        fEnt['fn'] = fn; // サイズ

        fEnt['size'] = file.size; // 単位表示付きサイズ
    ...
    key: "_getFileDataFromBData",
    value: function _getFileDataFromBData(bData) {
      var fileData = []; // ファイルデータ
      ...
      for (var i in bData) {
        ...
        fEnt['mime'] = bEnt.mime_type; // MIMEからファイル種別を判別する。
    ...
    key: "_validCheck",
    value: function _validCheck(fue_id, fileData) {
      ...
      for (var i in fileData) {
        var fEnt = fileData[i]; // バリデーション実行フラグがOFFならチェックをしない。
    ...
    key: "_getExtension",
    value: function _getExtension(fn) {
      if (fn == null) {
        return '';
      }
    ...
    key: "_validCheckMime",
    value: function _validCheckMime(fue_id, fEnt) {
    ...
    key: "_makeFileUnitHtml",
    value: function _makeFileUnitHtml(fue_id, fEnt, option) {
      ...
      var label_height = imgSize.height; // 画像要素と音楽要素の作成
      ...
      var paramData = [{
        ...
        'label': '更新日',
        'val': fEnt.modified,
        'flg': adf.modified_flg
      }]; // パラメータ区分のHTMLを組み立てる

      for (var i in paramData) {
        var pEnt = paramData[i];
        if (pEnt.flg != 1) continue;
        p_unit_html += "<div><label class='fuk_param_label'>" + pEnt.label + "</label>" + "<val class='fuk_param_val'>" + pEnt.val + "</val></div>";
      }
    ...
    key: "_getImgSize",
    value: function _getImgSize(fue_id, option) {
      var width = 0;
      var height = 0;
    ...
    key: "_clearBtnClick",
    value: function _clearBtnClick(e) {
    ...
    key: "_clearBtnAction",
    value: function _clearBtnAction(fue_id) {
      // クリアボタンラッパー要素を隠す
      var clearBtnW = this._getElement(fue_id, 'clear_btn_w');

      clearBtnW.hide(); // FU要素を取得し、中身をクリアする。

      var fue = this._getElement(fue_id, 'fue');

      fue.val(''); // 初期メッセージ要素を再表示する。

      var fukMsg = this._getElement(fue_id, 'fuk_msg');

      var fuk_msg_text = this.box[fue_id]['fuk_msg_text'];
      fukMsg.html(fuk_msg_text);
      fukMsg.show(); // プレビュー要素を取得し、中身をクリアする。

      var preview = this._getElement(fue_id, 'preview');

      preview.html(''); // 親ラベルの幅をautoから初期サイズに戻す
      ...
      parLabel.height(label_height); // ファイルデータもクリアする。
    ...
    key: "_cbAsynsEndInit",
    value: function _cbAsynsEndInit(count, option) {
      var pacb = function pacb() {};
    ...
    key: "_cbAsynsEndAction",
    value: function _cbAsynsEndAction(action) {
      switch (action) {
        case 'exe1':
    ...
    value: function uploadByAjax(callback, withData, option) {
      ...
      if (param.prog_slt) prog1 = this.unit.find(param.prog_slt); // AJAXによるファイルアップロード
      ...
      jQuery.ajax({
        ...
        try {
          ...
          jQuery(_this25.param.err_slt).html(str_json);
          return;
        }
        ...
        console.log(err_res);
        jQuery(_this25.param.err_slt).html(err_res);
        alert(statusText);
      });
    }
    /**
    ...
    key: "_getElement",
    value: function _getElement(fue_id, key) {
      var box = this.box; // データボックス

      if (box[fue_id] == null) box[fue_id] = {}; // ファイル要素を取得
    ...
    key: "_setToBox",
    value: function _setToBox(fue_id, box, option) {
      if (option == null) option = {};
      var bEnt = {};
      if (box[fue_id]) bEnt = box[fue_id]; // 親ラベル要素から幅を取得してセット

      var parLebel = this._getElement(fue_id, 'label'); // 親ラベル要素


      bEnt['label_width'] = parLebel.width();
      bEnt['label_height'] = parLebel.height(); // paramの初期メッセージテキストをセット。空ならFU要素のtitle属性をセット
      ...
      bEnt['fuk_msg_text'] = fuk_msg_text; // バリデーション情報をセットする
    ...
    key: "_convSizeUnit",
    value: function _convSizeUnit(value1, n) {
      if (n == null) n = 1;
      var res = '';
    ...
    key: "_createValidData",
    value: function _createValidData(valid_ext) {
      var validData = {
        'validExts': [],
        'validMimes': []
      };
      ...
      validExts = this._array_unique(validExts); // バリデーションMIMEを取得
    ...
    */
    ...
    key: "_getValidMimes",
    value: function _getValidMimes(validExts) {
    ...
    key: "_getMimeMapping",
    value: function _getMimeMapping() {
      var map = {};
      map['aac'] = 'audio/aac';
    ...
    key: "_escapeForAjax",
    value: function _escapeForAjax(data) {
      if (typeof data == 'string') {
        if (data.indexOf('&') != -1) {
          data = data.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&');
          return encodeURIComponent(data);
    ...
    key: "_stringRightRev",
    value: function _stringRightRev(s, mark) {
      if (s == null || s == "") {
        return s;
      }
...
var ImportFu = function ImportFu(param) {
  this.param = param; // ファイルアップロード要素のラッパー要素

  this.fu_wrap; // ファイルオブジェクト
  ...
  this.constract = function () {
    // If Param property is empty, set a value.
    this.param = _setParamIfEmpty(this.param); // ファイルアップロード要素をラベル等でラップ、および画像要素の追加

    _wrapFu(this.param); // ファイルアップロードイベントを追加する


    _addFileuploadEvent(this.param); // DnDイベントを追加する（DnD→ドラッグアンドドロップイベント）
  ...
  function _wrapFu(param) {
    ...
    fu.hide(); // ファイルアップロード要素をラベル要素でラッピングする。

    fu.wrap("<div id='" + param.wrap_xid + "'  ><label id = '" + param.label_xid + "' for='" + fu_xid + "' ></label></div>"); // ラッパー要素へCSSスタイルを適用する

    var fu_wrap = jQuery('#' + param.wrap_xid);
    fu_wrap.css(param.fuWrapCss); // 見出しを追加する
    ...
    caption.css(param.captionCss); // 背景画像要素を追加する
    ...
    var preview_html = "<div id='" + param.preview_xid + "' style='display:none;margin:0px 20px 20px 20px;'>" + "	<div id='" + param.pre_fn_xid + "' class='imp_fu_pre_fn'></div>" + "	<div class='imp_fu_pre_btns' style='margin-top:10px'>" + "		<input type='button' class='imp_fu_pre_exe_btn btn btn-success btn' value='アップロード' />" + "		<input type='button' class='imp_fu_pre_cancel_btn btn btn-default btn' value='キャンセル' />" + "	</div>" + "</div>";
    fu_wrap.append(preview_html); // ローディングGIFを埋め込む

    var gifBase64 = _getLodingGifBase64();

    var loding_gif = "<div class='loding_gif' style='display:none;padding:20px;'>" + "	<img src='" + gifBase64 + "' /> 読み込み中・・・" + "</div>";
    fu_wrap.append(loding_gif); // プレビュー要素のアップロード実行ボタンにイベントを組み込む
    ...
    fu_wrap.after(suc_msg_html); // エラー要素を追加する
  ...
  function _uploadByAjax(files) {
    ...
    var oFile = files[0]; // IEはファイル要素にnullが入っていてもチェンジイベントが発生するため対策する。
    ...
    var reader = new FileReader();
    reader.readAsDataURL(oFile); //ファイル読込成功イベント
    ...
    reader.onload = function (evt) {
      ...
      var fu_slt = param.fu_slt;
      var fd = new FormData();
      fd.append("upload_file", jQuery(fu_slt).prop("files")[0]); // WordPressのAjaxで必要なパラメータをセットする
      ...
      jQuery.ajax({
        ...
        try {
          data = JSON.parse(str_json); // エラーメッセージが空である場合（成功時）
  ...
  function _showPreview(files) {
    var param = self.param; // プレビュー要素を表示する

    var preview = self.fu_wrap.find('#' + param.preview_xid);
    preview.show(); // プレビュー要素にファイル名を表示する

    var imp_fu_pre_fn = preview.find('.imp_fu_pre_fn');
    var fn = files[0].name;
    imp_fu_pre_fn.html(fn); // 見出しを隠す

    var imp_fu_caption = self.fu_wrap.find('.imp_fu_caption');
    imp_fu_caption.hide(); // ファイルオブジェクトをメンバへセット（アップロード実行ボタンで必要になる）

    self.files = files; // 成功メッセージを隠す
  ...
  function _showLoading() {
    var param = self.param; // プレビュー要素を隠す
...
function errShowDetail() {
  ...
  if (!$('#err')[0] || !$('#detail_div')[0]) return; // エラーメッセージが空でない場合、詳細要素を表示します。
...
function moveClmSorterBase(page_code) {
  ...
  var csh_u = encodeURIComponent(csh_json); //列並替画面に遷移する。
...
function linkByRowdata(thisElm, base_url, field) {
  ...
  var tr = thisElm.parents('tr'); //	TR要素からフィールド要素を取得する

  var fElm = tr.find('.' + field); //	フィールド要素から可変値を取得する

  var v = fElm.text(); //	基本URLに可変値を置換適用。

  base_url = base_url.replace('%0', v); //	画面遷移
...
var LiviPage = function LiviPage() {
  this.execution = function (p_cssData) {
    //ツールチップ用DIV
    $(document.body).append("<div id='livipage_tooltip'></div>"); //デフォルトCSSデータ
    ...
    var cssData = {
      ...
      'width': '280px',
      'height': '460px',
      'overflow-y': 'auto' //引数CSSデータが空でなければ、CSSデータにマージする
    ...
    $('.livipage').click(function () {
      //対象セレクタ
      var slt = $(this).attr('href'); //対象要素の右上位置を取得

      var offset = $(this).offset();
      var left = offset.left;
      var top = offset.top; //対象要素の外幅を取得

      var width = $(slt).width();
      var height = $(slt).height();
      var tt_html = $(slt).html(); //リンクを作成する

      var link_text = $(this).html();
      var link1 = "<a href='" + slt + "'>" + link_text + "</a><br>";
      tt_html = link1 + tt_html;
      $('#livipage_tooltip').show();
      $('#livipage_tooltip').css(cssData); //ツールチップにHTMLをセット

      $('#livipage_tooltip').html(tt_html); //ツールチップの位置を算出

      var tt_left = left;
      var tt_top = top + 16; //ツールチップ要素に位置をセット

      $('#livipage_tooltip').offset({
        'top': tt_top,
        'left': tt_left
      });
      return false; //リンク無効
...
var ProcessWithMultiSelection = function ProcessWithMultiSelection(param) {
  this.param = param;
  ...
  this.action = function (kind_no) {
    ...
    var ids = getIdLintInChecked(); // IDリストが0件なら処理抜け
    ...
    $.ajax({
      ...
      cache: false,
      dataType: "text",
      success: function success(res, type) {
        if (res == 'success') {
          // ブラウザをリロードする
          location.reload(true);
        } else {
  ...
  function getIdLintInChecked() {
    ...
    $(slt).each(function () {
      var tr = $(this); // TR要素内からname属性またはclass属性を指定してチェックボックス要素を取得する

      var cb = getElmByNameOrClass(tr, myself.param.cb_slt); // チェックされている場合のみIDを取得してリストに追加する
  ...
  this.switchAllSelection = function (triggerCb) {
    ...
    var trigChecked = trigCb.prop('checked'); // 一覧をループして全行のチェック切替を行う

    var slt = myself.param.tbl_slt + ' tbody tr';
    $(slt).each(function () {
      var tr = $(this); // TR要素内からname属性またはclass属性を指定してチェックボックス要素を取得する

      var cb = getElmByNameOrClass(tr, myself.param.cb_slt); // チェックを切り替える
