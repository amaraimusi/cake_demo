...
jQuery(()=> {
	...
	$('#config_x_tbl').show();// 高速表示のためテーブルは最後に表示する
...
//			{'field':'config_x_group', 'inp_type':'select', 'list':config_xGroupList, 'def':2}, 
//			{'field':'config_x_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'config_x/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーした設定名、設定数値を貼り付けてください。（タブ区切りテキスト）\n(例)\n設定名A\t100\n設定名B\t101\n",
		}
	);
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('config_x_date');
}
