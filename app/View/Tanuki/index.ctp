...
$cssList[] = 'Tanuki/index.css' . $ver_str; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = CrudBaseU::getJsList();
$jsList[] = 'Tanuki/index.js' . $ver_str; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<!-- 検索条件入力フォーム -->
	<form action="" class="form_kjs" id="tanukiIndexForm" method="post" accept-charset="utf-8">
		
		<?php $this->CrudBase->inputKjMain('kj_main','',null,'タヌキ名、備考を検索する');?>
		...
		$this->CrudBase->inputKjNumRange('neko_val','neko_val'); 
		$this->CrudBase->inputKjText('kj_neko_name','neko_name');
		$this->CrudBase->inputKjMoDateRng('kj_neko_date','neko_date');
		$this->CrudBase->inputKjSelect('kj_neko_group','猫種別', $masters['nekoGroupList']); 
		$this->CrudBase->inputKjOuterId('kj_en_sp_id','絶滅危惧種ID'); 
		$this->CrudBase->inputKjText('kj_neko_dt','neko_dt');
	...
	<div id="main_tools" style="margin-bottom:10px;margin-top:4px">
		<div style="display:inline-block;width:75%; ">
			<?php 
				...
				// CSVエクスポート機能
				$csv_dl_url = "tanuki/csv_download?csrf_token={$csrf_token}";
...
<!-- 一覧テーブル -->
<table id="tanuki_tbl" class="table table-striped table-bordered " style="display:none;margin-bottom:0px">
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			...
			<?php $this->CrudBase->formOuterName('en_sp_id', '絶滅危惧種ID', 'ni'); ?>
		</div>
		<div class="cbf_inp_wrap">
	...
	<tr id="ajax_crud_edit_form" class="crud_base_form" style="display:none"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >絶滅危惧種ID: </div>
			<?php $this->CrudBase->formOuterName('en_sp_id', '絶滅危惧種ID', 'edit'); ?>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >neko_dt: </div>
			<div class='cbf_input'>
